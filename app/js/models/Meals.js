...
(function() {
	...
	app.models.Meals = {
		...
		loadFromJSON : function(date) {
		  ...
		  return dojo.xhrGet({
		    url : './app/resources/data/schemas/meals.json',
		    handleAs : 'json',
		    load : function(data) {
		...
		//return all of the current store's meals in this type (breakfast, lunch, etc)
		getMealsByMealTime: function(type){
			return store.query({mealTime:type});
		},
		//return all of the current store's meals in this category (fruit, protein, etc)
		getMealsByCategory: function(category){
			return store.query(function(item){
			  return item.food.category == category;
			})
		},
		...
		save: function(date) { // Mohit: 1/20/2012 remove cycle to function argument
			app.models.DashboardData.saveData(date,"meal",store.query());	// Mohit: 1/20/2012 remove cycle to function argument
